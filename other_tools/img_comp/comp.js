...
display_comp = function( data ) {
		...
		for ( let i = 0; i < l; i++) {
				let tmpl = document.querySelector("#comp_line").content;
				tmpl = document.importNode( tmpl, true );
				let box = document.querySelector("#comp_box").content;

				let left = document.importNode(box, true)
				let right = document.importNode(box, true)

				display_block( left, data.left.length > i ? data.left[i] : null );
				display_block( right, data.right.length > i ? data.right[i] : null );

				tmpl.querySelector('.leftbox').appendChild( left );
				tmpl.querySelector('.rightbox').appendChild( right );
				b.append( tmpl );
		}
}
